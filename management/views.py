...
from customer.views import Total
...
def About(request):
    ...
    if request.user.is_authenticated:
        ...
        d = {'team':team, 'total':total, 'Cartdish':Cartdish}
...
def Menu(request):
    ...
    if request.user.is_authenticated:
        Cartdish, total = Total(request.user)
        d = {'cat':cat, 'dishes':dishes, 'total':total, 'Cartdish':Cartdish}
...
def EditTeam(request):
    ...
    if 'add' in request.POST:
        ...
        img = request.FILES['img']
        Team.objects.create(name=name, designation=des, fb=fb, insta=insta, twitter=twt, img=img)
    if 'delete' in request.POST:
